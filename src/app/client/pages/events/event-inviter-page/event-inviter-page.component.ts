...
export class EventInviterPageComponent implements OnInit, OnDestroy {
  ...
  submitData(data, publishEvent?: boolean) {
    ...
    this.eventsService.inviteToEvent(this.event.id, invitesObject).subscribe(() => {
      if (publishEvent) {
        this.organizationService.bulkEventsEdit(this.organization.id, [{
  ...
  getLeagueSeasons() {
    ...
    this.organizationService.getOrganizationSeasons(this.organization.id, true).subscribe((response) => {
  ...
  getRegistrationStatusText(season: RCLeagueSeason) {
    ...
    if ((season.seasonTiming === 'future' || season.seasonTiming === 'current') &&
      season.registrationTiming !== 'close' && season.registrationTiming !== 'ended') {
      ...
      (season.seasonTiming === 'future' && season.registrationTiming === 'ended')
    ) {
  ...
  updateTextFilter(event) {
    ...
    this.seasons = this.temp.filter(function(d) {
      return (d.name && d.name.toLowerCase().indexOf(val) !== -1 ||
        (d.seasonLeague && d.seasonLeague.name.toLowerCase().indexOf(val) !== -1)) || !val;
    });
