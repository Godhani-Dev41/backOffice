...
export class AuthenticationService {
  ...
  login(email: string, password: string): Observable<RCOrganization> {
    ...
    return new Observable<RCOrganization>(subscriber => {
      p.subscribe((res: any) => {
        ...
        this.fetchActiveOrganization().subscribe(o => {
          subscriber.next();
          subscriber.complete();
   ...
   * @returns {Observable<RCOrganization>}
   */
  signup(data: { email: string, password: string, firstName: string, lastName: string, backoffice?: boolean }): Observable<any> {
    data.backoffice = true;

    return this.http
      .post<any>(environment.CS_URLS.API_DOMAIN + '/auth/signup', data).pipe(
      map(res => res),
      map((res: any) => {
        if (res.token) {
          this.setToken(res.token);
  ...
  fetchActiveOrganization(): Observable<RCOrganization> {
    return this.organizationsService.getUserOrganizations().pipe(map(response => {
  ...
  requestPasswordReset(email: string): Observable<any> {
    ...
    return this.http
      .get<any>(`${environment.CS_URLS.API_DOMAIN}/auth/password-reset?email=${encodeURIComponent(email)}&url=${encodeURIComponent(environment.SITE_URL)}`).pipe(
      map(res => res));
  }
  ...
  updateUserProfile(userId: string, data: { profile: any }): Observable<any> {
    return this.authHttp
      .put(`${environment.CS_URLS.API_ROOT}/users/${userId}/settings`, data).pipe(
      map(res => res));
  }
  ...
  getOrCreateUser(email: string, firstName: string, lastName: string, phoneNumber: string): Observable<any> {
    ...
    return this.http
      .post<any>(environment.CS_URLS.API_DOMAIN + '/v1/users/get-create-customer-by-email', { email, firstName, lastName, phoneNumber }).pipe(
          map((response) => response));
  }  
}
