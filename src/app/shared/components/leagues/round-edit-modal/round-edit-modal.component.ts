...
export class RoundEditModalComponent implements OnInit {
  ...
  onSubmit(data) {
    ...
    asyncJS.eachSeries(data.rounds, (item, next) => {
      if (item.isNew) {
      ...
      asyncJS.eachSeries(this.removeArray, (removeItem, nextItem) => {
  ...
  updateRound(roundId: number, roundName: string, ordinal: number, cb: Function) {
    this.leaguesService.updateRound(roundId, {
      name: roundName,
      ordinal: ordinal
    }).subscribe(() => {
      cb();
  ...
  createRound(roundName: string, ordinal: number, cb: Function) {
    this.leaguesService.createRound({
      name: roundName,
      ordinal: ordinal,
